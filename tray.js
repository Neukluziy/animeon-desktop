...
if (cfg.theme === 'custom' && cfg.custom) {
  ...
  document.documentElement.style.setProperty('--al-rgb', `${Math.min(r + 60, 255)},${Math.min(g + 60, 255)},${Math.min(b + 60, 255)}`);
...
if (info?.version && ver) ver.textContent = 'v' + info.version;
